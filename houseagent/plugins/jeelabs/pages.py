...
def init_pages(web, coordinator, db):
    ...
    if hasattr(sys, 'frozen'):
        web.putChild("jeelabs_images", File(os.path.join(os.path.dirname(sys.executable), 'plugins/jeelabs/templates/images')))
    else:
        web.putChild("jeelabs_images", File(os.path.join('houseagent/plugins/jeelabs/templates/images')))
    web.putChild('jeelabs_devices', JeelabsDevices(coordinator, db))
    web.putChild('jeelabs_devices_view', JeelabsDevicesView(db, coordinator))


class JeelabsDevicesView(Resource):
    ...
    def finished(self, locations):
        ...
        if hasattr(sys, 'frozen'):
            ...
            template = Template(filename=os.path.join(os.path.dirname(sys.executable), 'plugins/jeelabs/templates/devices.html'), lookup=lookup)
        else:    
            lookup = TemplateLookup(directories=['houseagent/templates/'])
            template = Template(filename='houseagent/plugins/jeelabs/templates/devices.html', lookup=lookup)
    ...
    def render_GET(self, request):
        ...
        # Uglyness alert!
        plugins = self.coordinator.get_plugins_by_type("Jeelabs")
        if len(plugins) == 0:
            self.request.write(str("No online Jeelabs plugins found..."))
            self.request.finish()
        elif len(plugins) == 1:
